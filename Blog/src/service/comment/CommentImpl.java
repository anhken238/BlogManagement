...
public class CommentImpl implements IComment {
    ...
    @Override
    public List<Comment> getAllCommentByUserOrPost(String keyword) {

        sql = "select * from comments" +
                " inner join users on users.id = comments.idUsers" +
                " inner join posts on posts.id = comments.idPosts where users.username like '%" + keyword + "%' or posts.title like  '%" + keyword + "%'";
        List<Comment> commentList = new ArrayList<>();
        try {
            statement = connection.createStatement();
            resultSet = statement.executeQuery(sql);
...
//    @Override
//    public List<Comment> getAllCommentByUserOrPost(String keyword) {
//        List<Comment> searchResult = new ArrayList<>();
...
//    }
    ...
    @Override
    public int countComments(int idPost) {
        int totalCommentofPost = 0;
        sql = "select count(comments.id) from comments where comments.idPosts =  '" + idPost + "'";
        try {
            statement = connection.createStatement();
        } catch (SQLException e) {
            e.printStackTrace();
        ...
        try {
            resultSet = statement.executeQuery(sql);
            while (resultSet.next()){
                totalCommentofPost = resultSet.getInt(1);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }

        return totalCommentofPost;
    }
